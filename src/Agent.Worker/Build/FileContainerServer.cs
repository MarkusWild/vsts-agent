...
namespace Microsoft.VisualStudio.Services.Agent.Worker.Build
{
    public class FileContainerServer
    {
            ...
            CancellationToken cancellationToken)
        {
            ...
            using (_uploadCancellationTokenSource = CancellationTokenSource.CreateLinkedTokenSource(cancellationToken))
            {
                ...
                try
                {
                    ...
                    await Task.Delay(TimeSpan.FromMinutes(1), _uploadCancellationTokenSource.Token);
        ...
        {
            while (!_uploadFinished.Task.IsCompleted)
            {
        ...
        private void UploadFileProgressReportReceived(object sender, ReportProgressEventArgs e)
        {
            ...
            int totalSize = e.TotalChunks * 16;
            int finishedSize = e.CurrentChunk * 16;
            progressQueue.Enqueue(StringUtil.Loc("ChunkProgressDetail", e.CurrentChunk, e.TotalChunks, e.File, finishedSize, totalSize));
        }
    }
}